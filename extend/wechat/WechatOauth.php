...
class WechatOauth
{
    ...
    public function __construct($config) {
        $this->wechat_config = $this->wechatConfig($config);
    }
    /**
     * 获取秘钥配置
     * @return [type] 数组
    ...
    public function wechatConfig($config) {
        $wechat_config = array_merge($this->wechat_config,$config);
     ...
     * @return string|mixed
     */
    public function getUserAccessUserInfo($code = "", $state = null)
    {
        if(empty($code)){
            $config = Config();
            $baseUrl = $config['url']['ali_weichat_author'];
//            $baseUrl = request()->url(true);
            $url = $this->getSingleAuthorizeUrl($baseUrl, $state);
            Header("Location: $url");
            exit();
        }else{
//            $cahche = Cache::get('access_token');
//            if($cahche) {
//                return $this->getUserInfo($cahche);
//            }
    ...
    public function getSingleAuthorizeUrl($redirect_url = "",$state = '1') {
        ...
        //用户授权
        return "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $this->wechat_config['appid'] . "&redirect_uri=".$redirect_url."&response_type=code&scope=snsapi_userinfo&state={$state}&connect_redirect=1#wechat_redirect";
        //静默授权
//        return "https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $this->wechat_config['appid'] . "&redirect_uri=".$redirect_url."&response_type=code&scope=snsapi_base&state={$state}&connect_redirect=1#wechat_redirect";
    }
    /**
     * 获取token
     * @return [type] 返回token
    ...
    public function getSingleAccessToken($code) {
        ...
        if(!isset($access_token['openid']) || !isset($access_token['access_token'])) {
            jsonCrypt(0, '授权信息获取失败');
        }
//        Cache::set('access_token', $access_token, 60);
        return $access_token;
    }
